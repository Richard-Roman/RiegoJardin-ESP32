/*
===============================================================================
Sistema de monitorización y riego automático con ESP32
===============================================================================

Código del proyecto desarrollado con ESP32. Permite gestionar
sensores de humedad y temperatura, controlar el riego mediante un
módulo de relé y se conecta a MQTT para la transmisión y recepción de datos.

Autor: [Richard Adan Roman Tocto]
Fecha: [3/02/2024]
Licencia: [GPL - 3.0]

===============================================================================
*/

#include <WiFi.h> // conectar a wifi
#include <PubSubClient.h> // Publicar y suscribir en mqtt
#include <DHT.h> // configuracion del DHT.
#include <WiFiClientSecure.h> // Conexion a internet segura.
#include <LiquidCrystal.h> // Display LCD.

// CONFGIGURAMOS LA CONEXION A INTERNET
const char* ssid = "Abner"; // Nombre del wifi
const char* password = "Abner123"; // Clave del wifi
WiFiClientSecure espClient; 

// CONFIGURAMOS CONEXION AL BROKER - SERVIDOR MQTT
const char* mqtt_server = "cdb39d727533450abfa0d803133c8e6b.s2.eu.hivemq.cloud"; // URL del Cluster
const char* usernameMQTT = "esp32"; // Cliente MQTT
const char* passwordMQTT = "123456789"; // Clave del cliente MQTT
const int mqtt_port = 8883; // Puerto

PubSubClient client(espClient);
unsigned long lastMsg = 0;
#define MSG_BUFFER_SIZE  (50)
char msg[MSG_BUFFER_SIZE];
int value = 0;
String Estado;
int Accionador;

static const char *root_ca PROGMEM = R"EOF(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)EOF"; /// Certificado de seguridad

// CONFIGURAMOS SENSOR DE TEMPERATURA - HUMEDAD 
#define DHTPIN 26 // Definimos pin del Sensor
#define DHTTYPE DHT11 // Definimos el modelo del Sensor
DHT dht(DHTPIN, DHTTYPE); // Configuramos el sensor con los datos de las dos lineas anteriores y le damos el nombre "dht"


/// CONFIGURAMOS PIN DEL MOTOR
const int MOTOR = 0;

/// CONFIGURAMOS PIN DEL SENSOR DE HUMEDAD DE SUELO
const int SUELO = 33;

/// CONFIGURAMOS LA PANTALLA LCD
LiquidCrystal lcd(22, 23, 5, 18, 19, 21);

void setup() {
  pinMode(MOTOR, OUTPUT); // Pin motor como Salida
  pinMode(SUELO, INPUT); // Pin Suelo como entrada
  pinMode(4, INPUT); // Pin Suelo como entrada
  dht.begin(); // Iniciamos el DHT
  Serial.begin(9600);
  setup_wifi();
  
  espClient.setCACert(root_ca);      // enable this line and the the "certificate" code for secure connection

  client.setServer(mqtt_server, mqtt_port); // Se conecta al servidor MQTT
  client.setCallback(callback); // Llamamos la funcion Callback
  client.subscribe("regado/motor"); // Se suscribe al topic "regado/motor"-----------------------------------------
  lcd.begin(16, 2);  // Inicializamos el display LCD
  digitalWrite(MOTOR, LOW); // Inicializamos el motor apagado
}

void loop() {
  
  // Probramos la conexion al Broquer MQTT 
  if (!client.connected()) {
    reconnect();
  }
  client.loop();

// Variables para leer los datos de temperatura y humedad
 float t = dht.readTemperature(); // Lee la temperatura
 float h1 = dht.readHumidity(); // Lee la humedad

 // HUMEDAD DEL SUELO
 int humedadLeida = analogRead(SUELO);
 int h2 = map(humedadLeida,2350,4095,100,0); /// Mapeamos los datos leidos.

 // MOSTRAMOS EN DISPLAY
 mostrar(t,h1,h2);

 // REGADO
 ControlRegado(h2, t);

 char temp[8]; // Variable auxiliar para guardar la temperatura
 dtostrf(t, 6, 2, temp);
 char hum1[8]; // Variable auxiliar para guardar la humedad
 dtostrf(h1, 6, 2, hum1);
 char hum2[8]; // Variable auxiliar para guardar la humedad del suelo
 dtostrf(h2, 6, 2, hum2);
 // Imprimmos por consola
 Serial.print("Valor leido: ");
 Serial.println(humedadLeida);
 Serial.print("Humedad del suelo: ");
 Serial.println(h2); 
 Serial.print("Temperatura: ");
 Serial.println(t); 
 Serial.print("Humedad del ambiente : ");
 Serial.println(h1); 
 delay(5000);
   
 if (!client.connected()) {
   reconnect();
  }   
  if (client.connected()){
    client.publish("sensor/temp", temp); // Publica el valor de la temperatura en el topic "sensor/temp"
    client.publish("sensor/hum1", hum1); // Publica el valor de la humedad en el topic "sensor/hum"
    client.publish("sensor/hum2", hum2); // Publica el valor de la humedad en el topic "sensor/hum2"
    Serial.println("se envio los datos al broker ");    
  }
 client.loop();
}


